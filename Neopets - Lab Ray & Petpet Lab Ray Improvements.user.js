...
function stylizeP2LabRats(reload = false) {
  ...
  if (petpetDivs.length != 0) {
    ...
    for (petpet of petpetDivs) {
      ...
      var truncate = petName.length > 10;
      ...
      if (!reload) {
        ...
        if (truncate) {
          var frames = ["0o0o","xXxX","xxxx","o0o0","XxXx",
            						"0o0","xXx","xxx","o0o","XxX",
                        "0o","xX","xx","o0","Xx"];
          var petNameTrunc = petName;

          var frame = frames.map(f => petName.startsWith(f) & petName.endsWith(f) ? f : "").filter(Boolean);
          if (frame.length > 0) {
            petNameTrunc = petName.substr(frame[0].length, petName.length - frame[0].length * 2);
          }
    			petNameTrunc = petNameTrunc.match(/[0-9]*(.*[^0-9]+)[0-9]*/)[1];
          petNameTrunc = petNameTrunc.match(/([^_]+).*/)[1];

          petNameSpan.innerText = petNameTrunc.substr(0,10);
        }
